...
async function validateAddNewPost(data) {
  const addNewPostSchema = Joi.object({
    ...
    category: Joi.string()
      ...
      .error(
        createHttpError.BadRequest("Enter the category ID correctly")
      ),
    text: Joi.string()
      .required()
    ...
    readingTime: Joi.number()
      .required()
      .error(
        createHttpError.BadRequest("Enter the post reading time correctly")
      ),
    type: Joi.string()
      .regex(/(free|premium)/i)
      .error(createHttpError.BadRequest("The post type is invalid")),
    related: Joi.array()
